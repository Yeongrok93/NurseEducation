...
class SupabaseLogRepository:
    ...
    def save_state(self, session_id: Optional[str], state: dict) -> None:
        if not self.client or not session_id:
            return

        try:
            self.client.table("sessions").update({"state": state}).eq("id", session_id).execute()
        except Exception:
            logger.exception("Failed to save game state.")

    def load_state(self, session_id: Optional[str]) -> Optional[dict]:
        if not self.client or not session_id:
            return None

        try:
            response = self.client.table("sessions").select("state").eq("id", session_id).execute()
            if response.data:
